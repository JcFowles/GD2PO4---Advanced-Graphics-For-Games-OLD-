...
class CubeApp : public WindowApp
{
public:
...
void CubeApp::updateScene(float dt)
{
	...
	float x = 0;// mRadius*cosf(0.5f*mTimer.getGameTime());
	float z = -5;// mRadius*sinf(0.5f*mTimer.getGameTime());
	float y = 5;// 50.0f*sinf(0.5f*mTimer.getGameTime()) + 50.0f;
	...
	D3DXMatrixRotationY(&mWorld, 1.0f);
	int c = 7;
}
...
void CubeApp::buildFX()
{
	...
	mfxWorldVar = mFX->GetVariableByName("mWorld")->AsMatrix();
}

void CubeApp::buildVertexLayouts()
{
