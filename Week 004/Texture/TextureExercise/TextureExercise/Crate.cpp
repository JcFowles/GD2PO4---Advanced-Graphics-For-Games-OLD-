...
private:
	...
	ID3D10ShaderResourceView* mFireMap[120];
	...
	ID3D10EffectShaderResourceVariable* mfxDiffuseMapVar2;
...
mfxLightVar(0), mfxDiffuseMapVar(0), mfxDiffuseMapVar2(0), mfxSpecMapVar(0), mfxTexMtxVar(0),
mVertexLayout(0), mDiffuseMapRV(0), mDiffuseMapRV2(0), mSpecMapRV(0), mEyePos(0.0f, 0.0f, 0.0f),
...
void CrateApp::initApp()
{
	...
	for (int i = 0; i < 120; i++)
	{
		std::string fileName = "FireAnim/Fire";
		int fileNum = i + 1;
		if (fileNum < 10)
		{
			fileName.append("00");
		...
		HR(D3DX10CreateShaderResourceViewFromFileA(md3dDevice,
			fileName.c_str(), 0, 0, &mFireMap[i], 0));
...
void CrateApp::drawScene()
{
	...
	mfxDiffuseMapVar->SetResource(mFireMap[change]);
...
void CrateApp::buildFX()
{
	...
	mfxDiffuseMapVar = mFX->GetVariableByName("gfireMap")->AsShaderResource();
